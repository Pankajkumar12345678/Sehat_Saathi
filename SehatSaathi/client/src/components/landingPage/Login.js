...
export default function Login(props) {
  ...
  const handleDoctorAdminLogin = async (email, password, path) => {
    ...
    if (data.err) {
      ...
      props.setToastShow(true);
      if (path === "http://localhost:5000/login/doctor") {
        navigate("/doctor/dashboard");
      } else {
        navigate("/admin/dashboard");
      }
  ...
  const handleLogin = async (e) => {
    ...
    switch (Toggle) {
      case "Patient":
        ...
        break;
      case "Doctor":
        handleDoctorAdminLogin(username, password, "http://localhost:5000/login/doctor");
        break;
      case "Admin":
        handleDoctorAdminLogin(username, password, "http://localhost:5000/login/admin");
        break;
      default:
        break;
    }
